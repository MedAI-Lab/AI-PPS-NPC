...
def Train_info_load(model_folder):
    ...
    Feature_path = os.path.join(model_folder, 'radiomic_feature_select_info.csv')
...
if __name__ == '__main__':
    ...
    model_folder = './RadiomicScoreCalculate/'
    New_data = 'test_radiomic_data.csv'
    predict_score, auc = New_data_predict(New_data, model_folder)
    pd.DataFrame(predict_score).to_csv((model_folder + 'results/score_predicted.csv'), header=['radiomic score'])
