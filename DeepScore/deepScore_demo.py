...
def Train_info_load(model_folder):
    ...
    Feature_path = os.path.join(model_folder, 'deep_feature_select_info.csv')
...
if __name__ == '__main__':
        ...
        model_folder = './DeepScoreCalculate/'
        New_data = 'test_deep_data.csv'
        predict_score, auc = New_data_predict(New_data, model_folder)
        pd.DataFrame(predict_score).to_csv((model_folder + 'results/score_predicted.csv'), header=['deep score'])
